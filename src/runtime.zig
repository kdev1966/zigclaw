...
test "NativeRuntime storage path contains zigclaw" {
    var native = NativeRuntime{};
    const rt = native.adapter();
    try std.testing.expect(std.mem.indexOf(u8, rt.storagePath(), "zigclaw") != null);
}
...
test "DockerRuntime filesystem with mount" {
    ...
    const rt = docker.adapter();
    try std.testing.expect(rt.hasFilesystemAccess());
    try std.testing.expectEqualStrings("/workspace/.zigclaw", rt.storagePath());
}
...
test "DockerRuntime filesystem without mount" {
    ...
    const rt = docker.adapter();
    try std.testing.expect(!rt.hasFilesystemAccess());
    try std.testing.expectEqualStrings("/tmp/.zigclaw", rt.storagePath());
}
...
test "NativeRuntime storage path" {
    var native = NativeRuntime{};
    const rt = native.adapter();
    try std.testing.expectEqualStrings(".zigclaw", rt.storagePath());
}
