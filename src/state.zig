...
//! Persisted to `~/.zigclaw/state.json` with atomic writes (temp + rename).
...
test "defaultStatePath" {
    const path = try defaultStatePath(testing.allocator, "/home/user/.zigclaw");
    defer testing.allocator.free(path);
    try testing.expectEqualStrings("/home/user/.zigclaw/state.json", path);
}
